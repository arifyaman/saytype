...
| `tests/cli.rs` | Binary-level contract tests for the offline checks (`--transcribe`/`--vad-test`/`--stream-test`) run against the real built binary: a wrong-rate (24 kHz) WAV and a missing WAV are rejected for every subcommand *before* any model load (an empty models dir makes the ordering observable: a model error would mean the WAV check no longer runs first) |
