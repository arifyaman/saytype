...
|---|---|
| `src/main.rs` | CLI dispatch (daemon / `--transcribe <wav>` / `--vad-test <wav>` / `--stream-test <wav>` / `--paste-test <text>`, all take `--asr`), models-dir resolution (`SAYTYPE_MODELS_DIR` env override > CWD `models/` > exe-dir `models/` > literal `models`), logging, and unit tests for the pure `--asr` value parsing (`parse_asr_value` valid/invalid mapping) + typing-mode flag parsing + the `resolve_models_dir` precedence chain (cwd-over-exe, non-dir skip, literal fallback) |
