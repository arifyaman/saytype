...
|---|---|
| `src/main.rs` | CLI dispatch (daemon / `--transcribe <wav>` / `--vad-test <wav>` / `--stream-test <wav>` / `--paste-test <text>`, all take `--asr`), models-dir resolution (`SAYTYPE_MODELS_DIR` env override > CWD `models/` > exe-dir `models/` > literal `models`), logging; the offline checks validate the WAV input (exists, 16 kHz) *before* loading any model, so a bad input fails in milliseconds; unit tests for the pure `--asr` value parsing (`parse_asr_value` valid/invalid mapping, repeated-flag last-wins order) + typing-mode flag parsing (including rejecting the mutually-exclusive `--live-typing` + `--no-live-typing` combination rather than silently picking one) + the `resolve_models_dir` precedence chain (cwd-over-exe, non-dir skip, literal fallback) |
| `src/daemon.rs` | D-Bus service (`Toggle`/`Stop`/`EraseWord`/`UndoErase` + signals), `Engine` state machine (Idle/Recording), per-session pipeline: `stream_task` (streaming) or `vad_task`+`asr_task` (batch), `injector_task` (owns the session's `Transcript`, applies erase/undo, emits `TranscriptUpdated`); `EngineEvent`/`AsrOutput`/`InjectorInput`; unit tests for `LiveTyping` stable-target logic (incl. multibyte prefix boundaries), the `Dictate::toggle` auto-repeat debounce, the Deferred-mode `injector_task` end-to-end event sequence (partials -> final -> erases -> the no-op stop paste, and an undo of a partial erase whose restored word still reaches the committed final), the Live-mode `injector_task` typing sequence against a faked `ydotool` (no typing before the stability threshold, capitalized prefix extension, backspace correction on a decoder revision, final convergence to the committed text, no stop paste), the Live-mode `injector_task` under a dead `ydotool` (every typing call fails: the session survives to the end, `typed` never advances so each retry re-types the full target, the final is marked `[type failed]` in its `SegmentTranscribed`, no stop paste), the Live-mode `injector_task` mid-dictation erase/undo (a working `ydotool`: erase backspaces the erased word out of the live-typed buffer, undo retypes the restored word at once, an unchanged stable target costs no ydotool call), and the FinalOnly-mode `injector_task` typing sequence (no partial typing even past the stability threshold, each final typed immediately as one chunk, partial-word erase/undo leaves the finals-only buffer untouched, committed-word erase backspaces exactly that word, no stop paste), and the `stream_task` streaming pipeline end to end on a gated real model + real speech (live partials while the utterance is in progress, all before a single `Final` committed at the utterance's trailing silence; skips when `models/` has no streaming backend or its test WAV), and the batch (Moonshine) pipeline end to end through the production `vad_task` -> `asr_task` wiring (exactly one `Final` per utterance, never a `Partial`; skips when `models/` has no Moonshine backend or a 16 kHz test WAV), and the `Engine` state-machine lifecycle end to end (idle `stop`/`erase`/`undo` are silent no-ops, a double start is rejected, and `stop()` drains the pipeline before `StateChanged("Idle")` so the session's state changes are exactly Recording-then-Idle; gated on `models/`, uses the real PipeWire capture when available, and sandboxes the stop-time paste to faked clipboard/ydotool tools so a live session can never touch the user's desktop), and a failed `start()` leaving no capture thread behind (the VAD model is corrupted on a temp-dir copy after `load`, so the load succeeds but the per-start VAD create fails; because `start()` creates the VAD before the capture thread exists, nothing is stranded, the failure is the clean corrupt-model error, and the engine starts a normal session immediately after the model is recovered; works on temp-dir copies so the shared `models/` and any live daemon are never touched) |
| `src/audio.rs` | PipeWire capture on a dedicated OS thread; S16LE 16 kHz mono in, f32 frames out via mpsc; pure conversion helpers `pcm_to_mono_f32` / `resample_linear` (a non-positive source rate is malformed and yields an empty frame instead of a "capacity overflow" panic - a zero rate would divide by zero and saturate the output length to usize::MAX, killing the capture thread) + unit tests (incl. the malformed-rate guard) |
| `src/vad.rs` | Silero VAD wrapper (both `check_model_file` and `Vad::new` run the corrupt-model guard first, so a corrupt `silero_vad.onnx` is a clean error at daemon start or session start, never a process abort), `detected()` in-progress probe, `VadParams` defaults, `AudioRing` context-padding buffer (batch path) + unit tests (incl. boundary/degenerate `slice` ranges; `AudioRing::new` clamps capacity to >= 1 so a zero-capacity ring can't panic on the modulo; corrupt-model checks: garbage and truncated-real files rejected with a re-download hint, `Vad::new` on a corrupt model returns a clean Err; gated real-model Silero tests that skip when `models/` is absent: silence never detected, speech detection + absolute segment start, `flush()` finalizing a trailing utterance) |
| `src/asr.rs` | `Asr` tri-backend (Nemotron streaming `OnlineRecognizer` preferred, Zipformer `OnlineRecognizer` fallback, Moonshine `OfflineRecognizer` batch), `BackendSelection`/`AsrKind`, `StreamingSession` (feed/partial/commit), batch `transcribe()` (with the `MIN_TRANSCRIBE_SAMPLES` 100 ms minimum-input guard: shorter input returns empty without running the model, avoiding Moonshine's onnxruntime "Invalid input shape" error noise on truncated WAVs), `polish()` (lowercase + strip sentence punctuation + online punct; intra-word hyphens/apostrophes preserved) output policy, optional `OnlinePunctuation` model auto-detect, + unit tests for the model-dir detection helpers (`find_moonshine`/`find_nemotron`/`resolve_zipformer_paths`/`find_online_punct` + filename matching/preference rules), `polish()` (sentence-punctuation-only strip: intra-word hyphens/apostrophes survive, the six sentence marks are dropped) with and against a real online-punct model (incl. the all-stripped-to-empty edge: a punct-only fragment must not make the model invent text out of an empty input), the Auto/Streaming fallback chain with partial (incomplete) model dirs, the `transcribe()` empty/short-input guard boundary (guarded vs. model path, on both FFI surfaces), the `transcribe()` real-audio batch path through the `OnlineRecognizer` branch (full utterance fed at once: non-empty text, model actually run), and a full `StreamingSession` round trip (feed/partial/commit/reset/reuse) on a real model + shipped test WAV - the real-model tests skip silently when `models/` is not installed and serialize under a shared lock (a Nemotron encoder alone is ~623 MB). Every discovered `.onnx` model file is envelope-checked (via `src/onnx.rs`) before the FFI create, so a corrupt model is a clean `CorruptModel` error that the Auto/Streaming fallback chain treats as a hard error (never a silent downgrade); a corrupt *optional* punct model degrades to a warning + no punct. Unit tests pin the corrupt/truncated-model clean errors for Nemotron, Moonshine (`.onnx`), and the punct degradation |
| `src/onnx.rs` | ONNX envelope plausibility check (`is_plausible_onnx`: first byte `0x08` = the `ir_version` field, then a streaming top-level protobuf walk that accepts the four real wire types (varint, fixed 64-bit, length-delimited, fixed 32-bit - incl. multi-byte varint tags/values) and rejects reserved wire types, any length running past EOF (truncation), and over-long varints) + `check_onnx_file` (actionable "corrupt or incomplete - re-run scripts/download-models.sh" error) + the `CorruptModel` marker error type (so backend fallback can distinguish *missing* model, a legitimate fallback, from *corrupt* model, a hard error). Exists because a corrupt model file makes the sherpa-onnx C++ loader throw a C++ exception across the FFI boundary, which aborts the whole process (SIGABRT, uncatchable in Rust) - verified for VAD, Nemotron, and Moonshine. `.ort` files (ORT binary format, Moonshine alt) are not checked - no cheap envelope exists for that format; unit tests cover synthetic valid/garbage/truncated/reserved-wire-type/multi-byte-varint/fixed-width(wire 1 & 5, incl. mid-payload truncation)/over-long-varint envelopes plus all real models in `models/` (gated) |
| `src/injector.rs` | ydotool typing (`type_text` - text always passed after the `--` end-of-options marker so a leading-dash word (hyphenated list item, "-20 degrees") is not parsed as a ydotool CLI option and silently dropped, `backspaces`), one-shot `paste_text` (clipboard via wl-copy/xclip + Ctrl+V, no-op on empty text), 5 s timeout around every subprocess, stdin-write EPIPE tolerance (a tool that exits before reading stdin reports its real exit/stderr error, not "Broken pipe"), `diff` prefix-diff, `capitalize_first` MVP punctuation stand-in + edge-case tests (multibyte/boundary `diff`, multi-char uppercase, no-op empty paste / zero backspaces, leading-dash text after `--`, clipboard tool fallback / xclip backgrounding / fast-fail error reporting, deferred-paste pipeline: session-tool-first clipboard order, exact `ydotool key ctrl+v`, no keystroke when the clipboard fails - faked wl-copy/xclip/ydotool on a sandboxed PATH, via the shared `src/testutil.rs` helpers also used by the daemon's Live-mode test) |
| `src/transcript.rs` | Pure `Transcript` state machine: single source of truth for the visible text (committed finals + live partial) with mid-dictation word erase/undo. Committed text is a token-slot sequence (permanently erased words are gaps, pending erasures hide a slot) so a restored word returns to its original position; partial erasures track token positions across decoder revisions and stay restorable for the whole utterance in progress regardless of how much the live hypothesis grows tick to tick, becoming permanent only at a genuine utterance boundary (this utterance's final showing content no partial ever did, or the next utterance's first partial); first visible word per segment is capitalized at render time; `display()` (HUD) + `buffer_target()` (target buffer) + unit tests (incl. empty-final edge cases: streaming `feed_final("")` drops dangling partial erasures but keeps pending committed ones and adds no segment, while `feed_batch_final("")` permanently finalizes pending erasures), an undo of a word the live decoder has since dropped is a visible no-op that still consumes the undo entry, an erased partial position tracks a final that replaces the word at that position (the erasure follows the position, undo restores the replacement word), a failed erase (fully hidden partial, nothing committed) is a no-op that pushes no undo entry, a committed erasure made within an utterance (after erasing the partial down) survives a shortening-revision final (no new content) still restorable, and the LIFO undo order across the partial-to-committed conversion at a final (a committed erasure made after partial ones restores first, then the partial words in reverse erase order)) |
| `src/testutil.rs` | `#[cfg(test)]` helpers shared by the subprocess-based unit tests (injector, daemon) and the real-model tests (asr, vad): fake `#!/bin/sh` tool writer, the PATH/WAYLAND_DISPLAY sandbox `EnvPatch` (prepends, restores both on drop), the crate-wide `TOOL_SPAWN_LOCK`/`ENV_LOCK` serialization locks (concurrent `#!/bin/sh` execs can ETXTBSY in the kernel's exec write-count bookkeeping, and process-global env must not leak between tests), and `MODEL_TEST_LOCK`/`model_lock()` + `repo_models_dir()` for the gated real-model tests (one model load at a time; skip when `models/` is absent) |
| `tests/cli.rs` | Binary-level contract tests run against the real built binary: a wrong-rate (24 kHz) WAV and a missing WAV are rejected for every offline check *before* any model load (an empty models dir makes the ordering observable: a model error would mean the WAV check no longer runs first), and the CLI exit-code contract (every misuse - missing argument, unknown argument, daemon positional, an `--asr` flag whose value is invalid, missing, or empty, and passing both mutually-exclusive `--live-typing` + `--no-live-typing` flags - exits 2 with a diagnostic before touching WAV/models, repeated `--asr` flags are last-wins and not a usage error (an invalid value on any occurrence still exits 2), `--help`/`-h` exits 0 listing every subcommand, and `--paste-test ""` exits 0 headlessly since empty text early-returns before spawning any tool) |
...
through `polish()` - lowercased, sentence punctuation stripped (intra-word
hyphens/apostrophes kept), then run through the optional online punct model
(`OnlinePunctuation`, auto-detected in `models/`) which restores casing +
punctuation. The Zipformer emits
...
`StateChanged("Idle")`; consumers may rely on that order. Every stage of
`stop()` is bounded: the join on the capture thread is conditional on its
observed exit (5 s timeout on its exit signal), so a capture thread that
never observes the stop flag (e.g. no capture source was ever scheduled)
cannot hang `stop()` and strand the final `StateChanged("Idle")`.
On the start side, `start()` creates the VAD **before** the capture thread
starts, so a failed `start()` (e.g. the VAD model corrupted after `load`)
cannot strand a live capture thread that no `stop()` could ever reach.
...
10. **Corrupt model files abort the process, not just error out.** A
    truncated or garbage `.onnx` model makes the sherpa-onnx C++ loader
    throw a C++ exception across the FFI boundary; Rust cannot catch
    foreign exceptions, so the whole process dies with SIGABRT (verified
    for the VAD, Nemotron, and Moonshine load paths). Every FFI model load
    is therefore preceded by the cheap `src/onnx.rs` envelope check
    (`check_onnx_file`); keep it that way when adding a model type. A
    discovered-but-corrupt model is a hard error in every `--asr` mode
    (the `CorruptModel` marker keeps the Auto/Streaming fallback chain
    from silently downgrading past a broken download). `.ort` files
    (Moonshine's ORT binary format) are the known unguarded case.

## Build, run, test

```sh
cargo build --release        # release binary at target/release/saytype
cargo test                   # unit tests (transcript incl. empty-final edge cases + mixed erase/undo order across the commit boundary + erase-boundary no-ops and shortening-final erase survival + position-tracking across a word-replacing final, injector incl. clipboard fallback + deferred-paste pipeline + leading-dash end-of-options typing, audio format conversion + malformed-rate resampling guard, VAD AudioRing + gated real-model Silero detection, ASR model detection + polish (incl. all-stripped-to-empty never invents text) + backend fallback + empty/short-input guard + real-model streaming round trip + real-audio OnlineRecognizer batch transcribe, ONNX envelope validation (all four protobuf wire types incl. fixed 32/64-bit, corrupt/truncated/over-long-varint model = clean re-download error, never a process abort) across VAD/ASR loads, daemon stable-target logic + toggle debounce + all three typing-mode injector sequences (live and final-only against a faked ydotool, incl. a dead-ydotool failure-resilience pass, Live-mode mid-dictation erase/undo editing the live buffer, and Deferred-mode undo of a partial erase before the final) + gated real-model streaming and batch pipeline tasks + engine lifecycle (idle no-ops, double-start guard, drain order, failed start leaves no capture thread and the engine stays usable), CLI --asr value parsing (incl. repeated-flag last-wins) + typing-mode flag parsing (incl. rejecting the mutually-exclusive --live-typing + --no-live-typing combination) + models-dir resolution precedence) + tests/cli.rs binary-level checks (wrong-rate/missing WAV rejected before any model load + CLI exit-code contract: misuse exits 2 incl. a missing/empty --asr value and contradictory typing-mode flags; repeated --asr flags are last-wins, not an error, --help exits 0, empty --paste-test exits 0 headlessly)
