...
| `src/onnx.rs` | ONNX envelope plausibility check (`is_plausible_onnx`: first byte `0x08` = the `ir_version` field, then a streaming top-level protobuf walk that handles multi-byte varint tags/values and rejects reserved wire types, any length running past EOF (truncation), and over-long varints) + `check_onnx_file` (actionable "corrupt or incomplete - re-run scripts/download-models.sh" error) + the `CorruptModel` marker error type (so backend fallback can distinguish *missing* model, a legitimate fallback, from *corrupt* model, a hard error). Exists because a corrupt model file makes the sherpa-onnx C++ loader throw a C++ exception across the FFI boundary, which aborts the whole process (SIGABRT, uncatchable in Rust) - verified for VAD, Nemotron, and Moonshine. `.ort` files (ORT binary format, Moonshine alt) are not checked - no cheap envelope exists for that format; unit tests cover synthetic valid/garbage/truncated/reserved-wire-type/multi-byte-varint/over-long-varint envelopes plus all real models in `models/` (gated) |
