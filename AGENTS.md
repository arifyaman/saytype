...
|---|---|
| `src/main.rs` | CLI dispatch (daemon / `--transcribe <wav>` / `--vad-test <wav>` / `--stream-test <wav>` / `--paste-test <text>`, all take `--asr`), models-dir resolution, logging, and unit tests for the pure `--asr` value parsing (`parse_asr_value` valid/invalid mapping) + typing-mode flag parsing |
