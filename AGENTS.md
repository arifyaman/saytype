...
| `src/injector.rs` | ydotool typing (`type_text`, `backspaces`), one-shot `paste_text` (clipboard via wl-copy/xclip + Ctrl+V, no-op on empty text), 5 s timeout around every subprocess, stdin-write EPIPE tolerance (a tool that exits before reading stdin reports its real exit/stderr error, not "Broken pipe"), `diff` prefix-diff, `capitalize_first` MVP punctuation stand-in + edge-case tests (multibyte/boundary `diff`, multi-char uppercase, no-op empty paste / zero backspaces, clipboard tool fallback / xclip backgrounding / fast-fail error reporting, deferred-paste pipeline: session-tool-first clipboard order, exact `ydotool key ctrl+v`, no keystroke when the clipboard fails - faked wl-copy/xclip/ydotool on a sandboxed PATH, via the shared `src/testutil.rs` helpers also used by the daemon's Live-mode test) |
