...
| `src/injector.rs` | ydotool typing (`type_text`, `backspaces`), one-shot `paste_text` (clipboard via wl-copy/xclip + Ctrl+V, no-op on empty text), 5 s timeout around every subprocess, `diff` prefix-diff, `capitalize_first` MVP punctuation stand-in + edge-case tests (multibyte/boundary `diff`, multi-char uppercase, no-op empty paste / zero backspaces) |
...
cargo test                   # unit tests (transcript incl. empty-final edge cases, injector, audio format conversion, VAD AudioRing, ASR model detection + polish + backend fallback, daemon stable-target logic + toggle debounce, config)
